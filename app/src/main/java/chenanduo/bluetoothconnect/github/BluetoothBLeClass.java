...
public class BluetoothBLeClass extends BleBase implements LeScanCallback {
    ...
    private void displayGattServices(BluetoothGatt gatt, String service_uuid, String notifi_uuid, String write_uuid) {
        ...
        //启用通知
        mBluetoothGatt.setCharacteristicNotification(mNotifiCharacteristic, true);
        //描述特诊和控制特诊的某些行为 BluetoothGattDescriptor  比如开启通知(我之前就遇到过设备通知是需要开启的)
    ...
    /*扫描结果  此方法应尽量避免耗时操作  synchronized*/
    @Override
    public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
        if (mBluetoothChangeListener != null) {
            mBluetoothChangeListener.onBleScanResult(device);
        }
    }
